...
async function upsertClusterWithArticle(cluster, article) {
    // update cluster fields conservatively
    if (!cluster)
        return null;
    // push article id if not present
    const idStr = article._id.toString();
    const existingIds = (cluster.articleIds || []).map((d) => d.toString());
    ...
    const artKeys = (article.keywords || []).map((k) => k.toLowerCase());
...
export async function clusterArticles(options) {
...
export async function clusterArticlesByIds(ids) {
    if (!ids || ids.length === 0)
        return { clustered: 0, created: 0 };
